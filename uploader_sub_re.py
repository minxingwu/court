...
class Uploader(Thread):
    ...
    def get_cached_token(self):
        try:
            with open(CACHE_FILE) as f:
                ...
                expire_time = token_json['expire_time']
                if time.time() < expire_time:
                    return self.token
                else:
                    raise Exception("token expired")
        except:
            return self.get_token_from_secret()
    ...
    def upload_from_url(self, path, target_path, resp):
        ...
        if os.path.splitext(path)[1] == '.mp4':
            ...
            content_type = 'image/png'
        data_vdo = {
            "Content-Type": (None, content_type),  # 此处为上传文件类型
    ...
    def upload_to_cloud(self, path, target_path):
        print("[uploader] uploading video file {} to cloud".format(path))
        resp = self.get_upload_token(target_path)
        success, video_info = self.upload_from_url(path, target_path, resp)
    ...
    def upload(self, task):
        ...
        #         path.split('/')[-1]
        # print(type)
        success, video_info = self.upload_to_cloud(type, str(venue_id_num)+ '/'+ 'highlight/'+ court_num + '/' + folder_name_datatime_str + '/' + path)
...
if __name__ == "__main__":
    ...
    folder_name_datatime_str = '20240518'
    ...
    if len(sys.argv) < 4:
        ...
        exit()
    print('start uploading clip')
    ori_video_name = sys.argv[1]
    event_timestamp = sys.argv[2]
    session_uuid = sys.argv[3]
    court_int_num = int(sys.argv[4])
    venue_id_num = 9

    # video_time = sys.argv[4]
    ...
    print('frame_count, fps', frame_count, fps)
    ...
    # url = "https://haoqiuwa-test-75067-5-1318337180.sh.run.tcloudbase.com/videos/event/v1"
    url = "https://golang-m7vn-53609-9-1318337180.sh.run.tcloudbase.com/videos/event/v1"
    # surroundings = 'cloud://test-5guvnyfne292a8b9.7465-test-5guvnyfne292a8b9-1318337180'
    ...
    payload = {
        ...
        "data":{
            "uuid":session_uuid,
            "court":court_int_num,
            "file_path":surroundings + '/' + str(venue_id_num) + '/' + 'highlight' + '/' + court_num + '/' + folder_name_datatime_str + '/' + video_name,
            "video_type":1,
            "hover_img_path":surroundings + '/' + str(venue_id_num) + '/' + 'highlight' + '/' + court_num + '/' + folder_name_datatime_str + '/' + picture_name,
            "time":int(duration),
            "team":"A"
        }
    }
    ...
    #     "data":{
    #         "uuid":session_uuid,
    #         "file_path":surroundings + '/' + 'highlight' + '/' + court_num + '/' + folder_name_datatime_str + '/' + picture_name,
    #         "relative_time":123123,
    #         "img_type":1
    #     }
    # }
    # response = requests.post(url,data=json.dumps(payload))
