...
def strip_optimizer(f: Union[str, Path] = 'all200.pt', s: str = '') -> None:
    """
    Strip optimizer from 'f' to finalize training, optionally save as 's'.

    Args:
        f (str): file path to model to strip the optimizer from. Default is 'all200.pt'.
    ...
    def __call__(self, epoch, fitness):
        ...
        if stop:
            LOGGER.info(f'Stopping training early as no improvement observed in last {self.patience} epochs. '
                        f'Best results observed at epoch {self.best_epoch}, all200 model saved as all200.pt.\n'
