...
def frame_predict_round(path, model=model):
    # path = "path/to/image"
    # results = model.predict(path, classes=1)
    results1 = model.predict(path, max_det=4, iou=0.4, classes=1)

    boxes = results1[0].boxes # boxes
    cls = results1[0].boxes.cls.cpu().numpy()     # class 类别
    xywhn = results1[0].boxes.xywhn.cpu().numpy() # x, y, width, height
    orig_img  = results1[0].orig_img  # 原始图片
    annotated_frame = results1[0].plot(probs=False) # 画框图
...
def crop_each_player(boxes, orig_img, save=False, name=None):
    # path = "path/to/image"    
    ...
    if type(res) == list:
        ...
        for item in res:
            if item.cls==1:  # only get person detect boxes
    ...
    if len(player)==2: # single player
        ...
        if save:
            ...
            cv2.imwrite(('/home/sportvision/Main_Session/img/crop_image_top-' + name + '.jpg'), a)
            cv2.imwrite(('/home/sportvision/Main_Session/img/crop_image_bottom-' + name + '.jpg'), b)
        ...
        if save:
            ...
            cv2.imwrite(('/home/sportvision/Main_Session/img/crop_image_top-' + name + '.jpg'), c_top)
            cv2.imwrite(('/home/sportvision/Main_Session/img/crop_image_bottom-' + name + '.jpg'), c_below)
