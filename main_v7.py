...
def make_parser():
    ...
    parser.add_argument('--polygon',
                    ...
                    type=str,
                    help='polygon',)
    parser.add_argument('--net_y_ratio',
                default=None,
                type=float,
                help='net_y_ratio',)
    parser.add_argument('--net_ball_threshold',
            default=None,
            type=int,
            help='net_ball_threshold',)
    parser.add_argument('--crop_frame',
            default=None,
            type=str,
            help='Crop region as a string, e.g., "[100:1080, 300:1600, :]"',)

    parser.add_argument('--uploader_url',
            default=None,
            type=str,
            help='uploader_url',)
    parser.add_argument('--uploader_surroundings',
            default=None,
            type=str,
            help='uploader_surroundings',)
    parser.add_argument('--cloudEnvId',
            default=None,
            type=str,
            help='cloudEnvId',)
    return parser
...
def main():
    ...
    net_y_ratio = args.net_y_ratio

    net_ball_threshold=args.net_ball_threshold

    crop_frame=args.crop_frame

    uploader_url = args.uploader_url
    uploader_surroundings = args.uploader_surroundings
    cloudEnvId = args.cloudEnvId
    process(
        ...
        net_ball_threshold=net_ball_threshold,
        crop_frame=crop_frame,
        uploader_url = uploader_url, 
        uploader_surroundings = uploader_surroundings, 
        ...
        while True: 
            ...
            if ret: 
                ...
                try:
                    ...
                    if window == [2,2,2,4] or window == [2,2,3,4] :
                        window.pop(0)
                        window.append(human_rects)
                    # print("human_rects is" , human_rects)
                    # draw_pic(cls, xywhn, annotated_frame, play_flag, human_rects, record_flag, session_num, test_num, window, polygon, net_y_ratio,net_ball_threshold)
                    ...
                    if play_flag == 1:
                        ...
                        if play_num > 2:
                            status = 0
                    if human_rects == 0 and record_flag == True:
                        status = 2
                    if play_flag == 3:
                        pass_num += 1
                        play_num = 0
                    ...
                    if (status_list[status] == "Playing") and (record_flag == False):
                        ...
                        Court_File_Name = f"{time_string}_the_{session_num}_court/"
                        ...
                        ffmpeg_cmd_court = ['ffmpeg',
                            ...
                            #   '-filter_complex', '[0:v]crop=1500:1000:60:0[v];[v][1:v]overlay=67.5:30[v];[v][2:v]overlay=1282.5:30',
                            '-filter_complex', '[0:v]crop=1920:1080:0:0[v];[v][1:v]overlay=67.5:30[v];[v][2:v]overlay=1482.5:30',
                            # '-filter_complex', '[0:v]crop=1920:1080:0:0[v];[v][1:v]overlay=67.5:30',
                            #   '-crf', '23',
                            # '-preset', 'veryfast',
                            '-c:v', 'h264_nvenc',
                            '-c:a', 'aac',
                    ...
                    if ( ((status_list[status] == "Freeing") or (status_list[status] == "Passing")) and (record_flag==True) ) or ((CutTheCourt == True) and (record_flag==True)):
                        ...
                        if p1 or p3:
                            ...
                            if time_diff > datetime.timedelta(minutes=3) :
                                ...
                                split_cmd = ['python', split_clip_path, str(now_save_court_video_path), uploader_origin_video_python_path, str(session_event_timestamp), session_uuid, str(session_start_time), 
                                             str(session_end_time),Court_Name, court_int_num, venue_id_num, str(polygon), str(net_ball_threshold), crop_frame, uploader_url, uploader_surroundings, cloudEnvId]
...
def draw_pic(cls, xywhn, annotated_frame, play_flag, human_rects, record_flag, session_num, test_num, window, polygon,net_y_ratio,net_ball_threshold):
    hei, wid, _ = annotated_frame.shape
    print('wid, hei is', wid, hei)
    line_color = (0, 0, 255)  # 线的颜色 (BGR 格式)
    line_thickness = 2  # 线的粗细
    start_point = (0, int(hei*net_y_ratio))  # 起始点坐标 (x, y)
    end_point = (wid, int(hei*net_y_ratio))  # 终止点坐标 (x, y)
    cv2.line(annotated_frame, start_point, end_point, line_color, line_thickness)
    cv2.line(annotated_frame, (0, net_ball_threshold), (1000, net_ball_threshold), line_color, line_thickness)
    ...
    line_thickness = 3
    text = "play_flag " + str(play_flag) + "   human_in_court " + str(human_rects)
    ...
    if record_flag:
        cv2.imwrite( (os.getcwd() +  f'/test_img/test_{session_num}_{test_num}_{window}.jpg' ) , annotated_frame)
    else:
        cv2.imwrite(os.getcwd() +  f'/test_img/test_{session_num}_{test_num}_not_in_play_{window}.jpg', annotated_frame)
